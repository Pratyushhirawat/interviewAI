...
function RoadmapResult({ roadmap, onClear }) {
  return (
    ...
    transition={{ duration: 0.3 }}>
        ...
        className='relative overflow-hidden bg-[#000000]/90 backdrop-blur-2xl border-white/10 rounded-2xl p-5 mb-5 shadow-[0_8px_32px_rgba(0,0,0,0.22)]'>
            ...
            <div className='relative flex items-start justify-between mb-4'>
              <div>
                ...
                <p className='text-sm text-white/45 mt-0.5'>Target: <span className='text-indigo-400 font-medium'>{roadmap.targetPackage}</span></p>
              </div>
              ...
              <div className='flex flex-col gap-2'>
                {
                  roadmap.modules.map((m, i) => (
                    <ModulesCard key={m.title} mod={m} index={i} /> 
                  ))
                }
              </div>
        </div>
