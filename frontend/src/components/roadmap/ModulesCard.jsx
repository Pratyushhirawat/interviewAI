...
const difficultyColor = { Easy: "#34d339", Medium: "#a78bfa", Hard: "#f87171"}

function ModulesCard({ mod, index }) {
  ...
  return (
    ...
    className='realtive overflow-hidden bg-[#000000]/90 backdrop-blur-2xl border border-white/10 rounded-xl cursor-pointer select-none shadow-[0_4px_18px_rgba(0,0,0,0.2)] hover:border-white/20 transition-all'>
        <div className='absolute inset-0 bg-linear-to-br from-white/7 via-transparent to-transparent pointer-events-none' />

        <div className='relative flex items-center gap-3 p-4'>
            <div className='w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold bg-white/5 border border-white/10' style={{ color: difficultyColor[mod.difficulty] }}>{index + 1}</div>
            ...
            <div className='flex items-center gap-2'>
                <span className='text-xs font-medium hidden sm:block' style={{ color: difficultyColor[mod.difficulty] }}>{mod.difficulty}</span>
                {open
                    ...
                    <div className='relative px-4 pb-4 pt-0 border-t border-white/8'>
                        ...
                        <div className='flex gap-2 flex-wrap'>
                            ...
                            <a href={mod.article} target='_blank' rel='noopener noreferrer' onClick={(e) =>e.stopPropagation()}>
                                ...
                                whileHover={{ scale: 1.03 }}
                                whileTap={{ scale: 0.97 }}
                                className='flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-white/10 text-white/45 bg-white/10 hover:bg-white/10 transition-colors'>
