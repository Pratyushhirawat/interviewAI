...
import { startInterview } from '../../apis/interviewApi'
...
function Setup({ user, setUser }) {
    ...
    const start = async () => {
        ...
        try {
            ...
            if (response) {
                try {
                    const coinResponse = await useCoins({ coins: 10, action: 'start-interview' })
    ...
    return (
        <div className='min-h-screen bg-white flex items-center justify-center p-3 sm:p-5'>
            <motion.div
                ...
                className='w-full max-w-4xl bg-[#0E1016] border border-white/10 rounded-2xl sm:rounded-3xl overflow-hidden grid lg:grid-cols-[40%_60%] shadow-[0_0_60px_rgba(255,255,255,0.03)]'>
                    ...
                    <div className='space-y-2 sm:space-y-3'>
                        {
                            [
                                ...
                                <motion.div key={index} whileHover={{ x: 4 }} className='flex items-center gap-3 rounded-xl border border-white/5 bg-white/3 p-3'>
                ...
                <div className='p-5 sm:p-7 flex flex-col'>
                    ...
                    <div className='mt-5 flex-1 space-y-4 overflow-y-auto'>
                        ...
                        <div className='rounded-xl border border-white/10 bg-[#17181E] p-4'>
                            <div className='flex items-center justify-between'>
                                ...
                                <button onClick={() => setUseResume(!useResume)} className={`relative shrink-0 w-10.5 h-6 rounded-full transition ${useResume ? "bg-white" : "bg-zinc-700"}`}>
                                    <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-black transition-all ${useResume ? "left-5" : "left-0.5"}`} />
                                </button>
                            </div>
                    ...
                    <motion.button
                        ...
                        className='mt-5 h-12 rounded-xl bg-white text-black tex-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-40 transition'>
