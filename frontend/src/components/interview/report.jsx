...
function Step3Report({ report, user, setUser }) {
  ...
  return (
    <div className='min-h-screen flex justify-center items-center bg-white md:p-5'>
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className='w-full max-w-5xl rounded-3xl bg-gray-100 border border-white/10 overflow-hidden'>
        ...
        <div className='p-8' ref={reportRef}>
          <div className='grid md:grid-cols-3 gap-5'>
            <div className='rounded-2xl bg-[#17181E] border border-white/10 p-6'>
              ...
              <h2 className='mt-2 text-4xl font-bold text-white'>{report.overallScore}
          ...
          <p className='mt-3 text-sm leading-7 text-zinc-400'>{report.summary}</p>
          </div>
              ...
              {report.strengths?.length > 0 ? (
                report.strengths.map((s)=> (
                  <div key={s} className='flex items-start gap-3 rounded-xl bg-green-500/5 border border-green-500/10 p-3.5'>
                    <div className='w-6 h-6 rounded-full bg-green-500 flex items-center justify-center shrink-0 text-xs'><FiCheck size={16} /></div>

                    <p className='text-sm text-zinc-300 leading-6'>{s}</p>
                  </div>
                ))
              ) : (
                <p className='text-zinc-500 text-sm'>No Strengths Available.</p>
              )}
            </div>
            </div>
              ...
              {report.weaknesses?.length > 0 ? (
                report.weaknesses.map((s)=> (
                  <div key={s} className='flex items-start gap-3 rounded-xl bg-red-500/5 border border-red-500/10 p-3.5'>
                    <div className='w-6 h-6 rounded-full bg-red-500 flex items-center justify-center shrink-0 text-xs'>!</div>

                    <p className='text-sm text-zinc-300 leading-6'>{s}</p>
                  </div>
                ))
              ) : (
                <p className='text-zinc-500 text-sm'>No Weaknesses Found.</p>
              )}
            </div>
            </div>
          </div>
          ...
          <div className='mt-5 space-y-3'>
            {
              report.recommendations?.length > 0 ? (
                report.recommendations?.map((r, i) => (
                  <div key={i} className='flex items-start gap-3.5 rounded-xl border border-white/10 bg-white/3 p-4
                  '>
                    <div className='w-7 h-7 rounded-full bg-white text-black flex items-center justify-center font-semibold shrink-0 text-sm'>{i+1}</div>
          ...
          <div className='mt-8'>
            <h3 className='text-xl font-bold text-black mb-5'>Question wise Analysis</h3>
              ...
              {report.questions?.map((item, index) => (
                <div key={index} className='rounded-2xl bg-[#17181E] border border-white/10 p-6'>
                  ...
                  <div className='mt-6'>
                    ...
                    <p className='mt-2.5 text-sm leading-6 text-zinc-300'> {item.userAnswer || "No answer submitted"} </p>
                  </div>
                  ...
                  <div className='grid md:grid-cols-4 gap-3.5 mt-6'>
                      ...
                      <h5 className='mt-2 text-xl font-bold text-white'>{ item.feedback?.relevace ?? 0 }</h5>
                      </div>
                    ...
                    <div className='mt-6'>
                        ...
                        <div className='mt-3.5 space-y-2.5'>
                          {item.feedback?.improvements?.map((item, i) => (
                            <div key={i} className='rounded-lg border border-white/10 bg-white/3 p-3.5'>
