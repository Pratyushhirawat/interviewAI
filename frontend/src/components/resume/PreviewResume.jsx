...
function PreviewResume({ data, onBack, user, setUser}) {
  ...
  return (
    <div className='min-h-screen text-[#0A0A0A] bg-white'>
        {/* Header */}
        ...
        <div className='mx-auto flex max-w-7xl flex-col gap-3 px-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5'>
            <div>
                ...
                <p className='mt-0.5 text-[10px] text-gray-400 sm:text-xs'>Review your resume before downloading</p>
            </div>

            <div className='flex items-center justify-between lg:justify-end gap-2.5'>
                <button onClick={onBack} className='flex h-8 items-center justify-center gap-1.5 rounded-lg border border-black/15 px-2.5 sm:px-3 text-xs text-black/60 transition hover:border-black/25 hover:text-[#0A0A0A]'>
                    <FiArrowLeft size={15} />
        ...
        <div className='overflow-auto bg-[#F8F9FA] px-3 py-4 sm:px-6 sm:py-8'>
            <div className='mx-auto flex justify-center'>
                <div style={{
                    ...
                    <div ref={resumeRef} className='rounded-md bg-white shadow-[0_0_50px_rgba(0,0,0,0.6)]'>
