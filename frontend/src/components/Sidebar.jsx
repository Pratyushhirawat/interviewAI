...
import { FaCirclePlus, FaCoins } from 'react-icons/fa6'
...
function Sidebar({
    ...
    const inner = (
        <div className='flex flex-col h-full'>
            <div className={`px-3 h-13 border-b border-black/8 shrink-0 flex items-center ${sidebarOpen ? "justify-between" : "justify-center"}`}>
            {sidebarOpen && (
                <div className='flex items-center gap-2.5'>
                    ...
                    className='font-extrabold text-sm tracking-tight text-[#0A0A0A] whitespace-nowrap'>InterviewAI</motion.span>
                </div>
            )}
            <div className='flex items-center'>
                ...
                className='hidden md:flex text-black/30 hover:text-[#0A0A0A] transition-colors shrink-0 cursor-pointer'>
                    <FiSidebar size={15} />
                ...
                className='md:hidden text-black/30 hover:text-[#0A0A0A] transition-colors shrink-0 cursor-pointer'>
                    <FiSidebar size={15} />
            ...
            <div className='px-2.5 pt-3 pb-1.5 shrink-0'>
                ...
                whileTap={{ scale: 0.97 }}
                className={`w-full flex items-center gap-2 bg-[#000000] text-white font-semibold rounded-lg py-2 cursor-pointer transition-all shadow-[0_4px_14px_rgba(0,0,0,0.25)] hover:bg-[#1a1a1a] ${sidebarOpen ? "px-2.5" : "justify-center px-0"}`}>
    ...
    <>
        ...
        <AnimatePresence>
            {mobileOpen && (
                ...
                exit={{ x: -280 }}
                transition={{ duration: 0.25, ease: 'easeInOut' }}
                className='fixed top-0 left-0 h-screen w-50 max-w-[85vw] bg-white border-r border-black/8 flex flex-col z-50 md:hidden overflow-hidden'>
                    {inner}
