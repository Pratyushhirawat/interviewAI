...
import { AnimatePresence, motion, renderSVG } from 'motion/react'
            ...
            const options = {
                ...
                handler: async function (response) {
                    try {
                        ...
                        console.log(error)
                        alert(error?.response?.data?.message || "Payment verfication failed")
                    }
                },
    ...
    <div className='min-h-screen bg-white text-[#0A0A0A]'>
        <motion.nav
                        ...
                        <div className='mx-auto flex h-12 max-w-7xl items-center justify-between px-3 md:px-5'>
                            ...
                            <button onClick={()=>setShowMenu(!showMenu)} className='flex w-8 h-8 items-center justify-center rounded-lg border border-black/15 text-black/60 transition hover:border-black/35 hover:text-[#0A0A0A]'>
                            ...
                            {showMenu && (
                                ...
                                <div onClick={()=>setShowMenu(false)} className='fixed inset-0 z-30 bg-black/20 lg:hidden' />
                                    ...
                                    <div className='absolute right-1 top-11 z-40 w-60 max-w-[calc(100vw-24px)] rounded-xl overflow-hidden bg-[#000000]/90 backdrop-blur-2xl border border-white/10 p-3.5 shadow-[0_8px_32px_rgba(0,0,0,0.25)]'>
                                    ...
                                    <div>
                                        <p className='text-[10px] text-white/40'>Available Coins</p>
                                    ...
                                    <div className='relative mt-3.5 space-y-1.5'>
                                        {[
                                            ...
                                            <div key={item.title} className='flex items-center justify-between rounded-lg bg-white/5 border border-white/8 px-2.5 py-1.5'>
                                                ...
                                                <span className='text-xs font-bold text-red-400'>{item.coin}</span>
                                            </div>
                                        ))}
                                    </div>

                                    <div className='relative mt-3.5 rounded-lg border border-indigo-400/20 bg-indigo-500/10 p-2.5'>
                                        <p className='text-[10px] leading-4 text-indigo-300'>Every AI feature uses Interview Coins. Buy more coins anytime to continue using Resume Builder, Resume Scorer, AI Interview and Roadmap Generator.</p>
                                    </div>
                                    </div>
                                </>
                            )}
                    ...
                    <div className='mx-auto max-w-4xl px-4 py-6'>
                        <div className='text-center'>
                            <h1 className='text-3xl font-bold text-[#0A0A0A]'>Interview Coins</h1>
                            <p className="mt-2 text-sm text-black/45">
                            Use coins for Resume Scoring, Resume Builder, AI Interview, and Roadmap Generation.</p>
                        </div>

                        <div className='mt-8 grid place-items-center gap-3 md:grid-cols-2'>
                            {plan.map((plan) => (
                                <PricingCard key={plan.title} {...plan} onBuy={()=>handlePayment(plan)} />
                            ))}
