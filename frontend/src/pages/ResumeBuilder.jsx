...
function ResumeBuilder({user, setUser}) {
  ...
  return (
    <div className='min-h-screen text-[#0A0A0A] bg-white flex flex-col'>
      <motion.nav
          ...
          <div className='mx-auto w-full max-w-2xl'>
            <div className='mb-4'>
              <div className='flex items-center justify-between mb-1.5'>
                <p className='text-[10px] text-black/40 font-medium'>Step {currentStep} of {TOTAL_STEPS}</p>
              ...
              <div className='h-full bg-[#0A0A0A] transition-all duration-300' style={{width: `${progressPCT}%`}} />
              </div>

              <div className='mt-3'>
                <h2 className='text-xl font-bold sm:text-2xl'>{activeStep.title}</h2>
                <p className='mt-1 text-black/45 sm:text-sm'>{activeStep.subtitle}</p>
              </div>
            </div>
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-medium border transition-all
              ...
              <div className='flex items-center gap-1.5'>
                {STEPS.map((s)=> (
                  <button key={s.step} onClick={() => setCurrentStep(s.step)} className={`rounded-full transition-all ${s.step === currentStep 
                    ? 'w-4 h-1.5 bg-[#0A0A0A]'
                    : s.step < currentStep
              ...
              {isLastStep ?
               (<button onClick={()=>setShowPreview(true)} className='flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold bg-[#0A0A0A]/90 backdrop-blur-2xl border border-white/10 text-white shadow-[0_8px_24px_rgba(0,0,0,0.25)] hover:border-white/20 transition-all'>
                <FiEye size={13} />
                <span className='hidden sm:block'>Previous Resume</span>
               </button>)
               : 
