...
const PACKAGE_OPTIONS = ["10 LPA", "15 LPA", "20 LPA", "25 LPA", "30 LPA", "35 LPA", "40 LPA", "45 LPA", "50 LPA", "55 LPA", "60 LPA", "65 LPA", "70 LPA", "75 LPA", "80 LPA", "1 CR+"]
...
function Roadmap({ user, setUser }) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            try {
                const coinResponse = await useCoins({ coins: 10, action: 'roadmap-builder' })
    ...
    return (
        <div className='min-h-screen bg-white text-[#0A0A0A] flex flex-col'>
            <motion.nav
                ...
                <div className='mx-auto flex h-12 max-w-7xl items-center justify-between px-3 md:px-5'>
                    <div onClick={() => navigate("/dashboard")} className='flex cursor-pointer items-center gap-1.5'>
                        <span className='text-sm font-extrabold sm:text-base text-[#0A0A0A]'>InterviewAI</span>
                        <span className='hidden rounded bg-black/5 px-1.5 py-0.5 text-[10px] text-black/50 sm:block'>Roadmap Builder</span>
                    </div>

                    <button onClick={()=>setHistoryOpen(!historyOpen)} className='flex h-8 items-center justify-center rounded-lg border border-black/15 text-black/60 gap-2 transition px-2 hover:border-[#0A0A0A] hover:text-[#0A0A0A] text-[10px]'>
                        <FiClock size={15} />
            ...
            <main className='flex-1 overflow-y-auto pb-28 sm:pb-32 pt-16 sm:pt-20'>
                <div className='max-w-7xl mx-auto px-3 sm:px-4 pt-5 sm:pt-6'>
                        ...
                        {!roadmap ? (
                            <motion.div
                                ...
                                <div className='mt-6 sm:mt-8 grid grid-cols-3 gap-2 sm:gap-3 w-full max-w-xs sm:max-w-sm'>
                                    {
                                        ["Frontend Developer", "Backend Engineer", "ML Engineer"].map((r) => (
                                            <motion.button
                                                key={r}
                                                onClick={() => setRole(r)}
                                                whileHover={{ y: -2 }}
                                                whileTap={{ scale: 0.97 }}
                                                className='relative cursor-pointer overflow-hidden text-xs py-2 sm:py-2.5 px-2 sm:px-3 rounded-xl bg-[#000000]/90 backdrop-blur-2xl border border-white/10 text-white/55 hover:text-white hover:border-white/25 shadow-[0_4px_14px_rgba(0,0,0,0.18)] transition-all'>
                                        ...
                                        className='absolute bottom-full mb-4 right-0 w-28 rounded-xl overflow-hidden border border-white/10 bg-[#0A0A0A] shadow-[0_8px_24px_rgba(0,0,0,0.35)] z-10'>
                                        {
                                            PACKAGE_OPTIONS.map((pkg) => (
                                                <button
                                                    ...
                                                    key={pkg} className={`w-full text-left text-xs px-3 py-2 transition-colors ${pkg === targetPackage
            ...
            <AnimatePresence>
                {
                    historyOpen && (
                        ...
                        className='fixed right-0 top-0 bottom-0 z-50 w-70 sm:w-75 max-w-[85vw] bg-white border-l border-black/8 flex flex-col overflow-y-auto shadow-[0_0_40px_rgba(0,0,0,0.1)]'>
                            <div className='flex items-center justify-between px-4 py-3 sm:py-4 border-b border-black/8 sticky top-0 bg-white/90 backdrop-blur-xl'>
                                ...
                                <button onClick={()=>setHistoryOpen(false)} className='text-black/35 hover:text-[#0A0A0A] transition-colors cursor-pointer'>
                                    <FiX size={16} />
                                </button>
                            </div>
                                ...
                                {historyLoading ? (
                                 ...
                                 (
                                    ...
                                    history.map((h, i) => (
                                        ...
                                        }} className='relative overflow-hidden text-left p-4 rounded-xl bg-black/90 border border-white/10'>
                                            <h3 className='text-white font-semibold'>{h.title}</h3>
                                            <div className='flex justify-between items-center my-1'>
                                                <span className='text-indigo-400 text-xs'>{h.targetPackage}</span>
                                                <p className='text-xs text-white/40'>{h.duration}</p>
                                            </div>

                                            <p className='text-xs text-white/30 mt-1'>{new Date (h.createdAt).toLocaleDateString()}</p>
                                        </button>
                                    ))
                                 )}
                            </div>
