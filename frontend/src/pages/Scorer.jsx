...
const Navbar = ({ label }) => {
    ...
    return (
        <motion.nav
            ...
            <div className='mx-auto flex h-12 max-w-7xl items-center justify-start px-3 md:px-5'>
...
function Scorer({ user, setUser }) {
    ...
    if (resume) return (
        <div className='text-[#0A0A0A] min-h-screen bg-white'>
            ...
            <section className='max-w-6xl mx-auto px-3 pt-18 sm:pt-20 pb-8 space-y-3.5'>
                ...
                <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
                    ...
                    <button onClick={() => dispatch(setResume(null))} className='text-[10px] sm:text-xs text-black/50 hover:text-[#0A0A0A] border border-black/15 hover:border-black/35 px-2.5 py-1 rounded-lg transition-colors'>Re-upload</button>
                </div>
                ...
                <motion.div
                    ...
                    <div className='relative'>
                        <p className='text-white/50 text-xs mb-0.5'>Resume Scorer</p>
                        <p className='text-lg sm:text-xl font-bold mb-1.5 text-white'>
                            {resume.score >= 85 ? "Strong" : resume.score >= 65 ? "Average" : "Needs Work"}
                        </p>
                        <div className='flex items-center gap-1.5'>
                            <FiUser className='text-indigo-400 text-xs' />
                            <span className='text-xs text-indigo-300'>{resume?.suggestedRole}</span>
                        </div>
                    </div>
                </motion.div>
                ...
                <div className='grid grid-cols-1 gap-3 md:grid-cols-2'>
                    <motion.div
                        ...
                        <div className='flex relative flex-wrap gap-1.5'>
        ...
        <div className='min-h-screen bg-white text-[#0A0A0A]'>
            ...
            <section className='flex min-h-screen items-center justify-center px-3 pt-18 pb-6'>
                ...
                <motion.div
                    ...
                    <p className='relative text-[10px] text-white/40 tracking-widest uppercase mb-1.5'>Step 1 tO 2 {resume?.name}</p>
                    ...
                    <p className='relative text-white/45 text-xs mb-4'>We'll score it and give you actionable feedback</p>

                    <label className={`relative flex flex-col items-center justify-center w-full h-40 sm:h-48 rounded-2xl border-2 border-dashed cursor-pointer ${file ? "border-white/40 bg-white/6" : "border-white/15 bg-white/3 hover:border-white/30"}`}>
                        ...
                        <p className='text-[10px] text-white/35 mt-1'>PDF Only - Max 200MB</p>
